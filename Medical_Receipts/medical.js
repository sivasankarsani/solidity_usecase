...
const contractJSON = require('./build/contracts/MedicalReceipt.json')
...
exports.getPatientReceipt = function (req,res) {
    contract.methods.getPatientLatestMedicalReceipt(addressFrom)
    .call()
    .then( (result, err) => {
        if(result) {
            res.json({
                "PatientReceipt" : result
            })
        }
        else {
            res.json({
  ...
  exports.setPatientData = function(req,res){ 
      web3.eth.getTransactionCount(addressFrom).then(txCount => {
        const txData = {
          ...
          from: addressFrom,
          value: 0x0,
          data: contract.methods.setPatientMedicalReceipt(req.body.precription,req.body.doctor,req.body.hospital).encodeABI()
        }
